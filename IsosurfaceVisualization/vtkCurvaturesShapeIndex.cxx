/*=========================================================================
  ...
  Module:    vtkCurvaturesShapeIndex.cxx
...
vtkStandardNewMacro(vtkCurvaturesShapeIndex);

//------------------------------------------------------------------------------
#if VTK3
vtkCurvaturesShapeIndex* vtkCurvaturesShapeIndex::New()
{
  // First try to create the object from the vtkObjectFactory
  vtkObject* ret = vtkObjectFactory::CreateInstance("vtkCurvaturesShapeIndex");
  if(ret)
    {
    return (vtkCurvaturesShapeIndex*)ret;
    }
  // If the factory was unable to create the object, then create it here.
  return new vtkCurvaturesShapeIndex;
}
#endif
//-------------------------------------------------------//
vtkCurvaturesShapeIndex::vtkCurvaturesShapeIndex()
{
  this->CurvatureType = VTK_CURVATURE_GAUSS;
  this->InvertMeanCurvature = 0;
}
//-------------------------------------------------------//
void vtkCurvaturesShapeIndex::GetMeanCurvature(vtkPolyData *mesh)
{
	// stephan meesters 12-5-2013, check if mean curvature is already computed
	if(mesh->GetPointData()->GetArray("Mean_Curvature") != NULL)
		return;

    vtkDebugMacro("Start vtkCurvaturesShapeIndex::GetMeanCurvature");
...
void vtkCurvaturesShapeIndex::GetGaussCurvature(vtkPolyData *output)
{
	// stephan meesters 12-5-2013, check if Gaussian curvature is already computed
	if(output->GetPointData()->GetArray("Gauss_Curvature") != NULL)
		return;

    vtkDebugMacro("Start vtkCurvaturesShapeIndex::GetGaussCurvature()");
    // vtk data
...
void vtkCurvaturesShapeIndex::GetMaximumCurvature(vtkPolyData *input,vtkPolyData *output)
{
...
void vtkCurvaturesShapeIndex::GetMinimumCurvature(vtkPolyData *input,vtkPolyData *output)
{
  this->GetGaussCurvature(output);
  this->GetMeanCurvature(output);
...
void vtkCurvaturesShapeIndex::GetShapeIndex(vtkPolyData *input,vtkPolyData *output)
{
	this->GetMaximumCurvature(input,output); // k1
	this->GetMinimumCurvature(input,output); // k2
	
	vtkIdType numPts = input->GetNumberOfPoints();
  
	vtkDoubleArray *shapeIndex = vtkDoubleArray::New();
	shapeIndex->SetNumberOfComponents(1);
	shapeIndex->SetNumberOfTuples(numPts);
	shapeIndex->SetName("Shape_Index");
	output->GetPointData()->AddArray(shapeIndex);
	output->GetPointData()->SetActiveScalars("Shape_Index");
	shapeIndex->Delete();

	vtkDoubleArray *minimum = static_cast<vtkDoubleArray *>(
		output->GetPointData()->GetArray("Minimum_Curvature"));
	vtkDoubleArray *maximum = static_cast<vtkDoubleArray *>(
		output->GetPointData()->GetArray("Maximum_Curvature"));
	double k1, k2, si;

	double halfPi = 0.5*vtkMath::Pi();
	for (vtkIdType i = 0; i<numPts; i++)
    {
		k1 = maximum->GetComponent(i,0);
		k2 = minimum->GetComponent(i,0);
		si = halfPi * atan( (k1 + k2) / (k1 - k2) );
		shapeIndex->SetComponent(i, 0, si);
	}
}

void vtkCurvaturesShapeIndex::GetCurvedness(vtkPolyData *input,vtkPolyData *output)
{
	this->GetMaximumCurvature(input,output); // k1
	this->GetMinimumCurvature(input,output); // k2
	...
	vtkDoubleArray *curvedness = vtkDoubleArray::New();
	curvedness->SetNumberOfComponents(1);
	curvedness->SetNumberOfTuples(numPts);
	curvedness->SetName("Curvedness");
	output->GetPointData()->AddArray(curvedness);
	output->GetPointData()->SetActiveScalars("Curvedness");
	curvedness->Delete();
	...
	for (vtkIdType i = 0; i<numPts; i++)
    {
		k1 = maximum->GetComponent(i,0);
		k2 = minimum->GetComponent(i,0);
		cur = 0.5 * sqrt(k1*k1 + k2*k2);
		curvedness->SetComponent(i, 0, cur);
	}
}

//-------------------------------------------------------
int vtkCurvaturesShapeIndex::RequestData(
  ...
  vtkInformationVector *outputVector)
{
  ...
  else if ( this->CurvatureType ==  VTK_SHAPE_INDEX )
    {
    this->GetShapeIndex(input, output);
    }
  else if ( this->CurvatureType ==  VTK_CURVEDNESS )
    {
    this->GetCurvedness(input, output);
    }
  else 
    {
...
/*-------------------------------------------------------*/
void vtkCurvaturesShapeIndex::PrintSelf(ostream& os, vtkIndent indent)
{
