...
//    double dimStep = 0.5;
//    int N = dims/dimStep;
//    //double* kernelOutput = (double*)malloc(pow(N,3)*numOrientations*sizeof(double));
...
//    return;
	...
	if(this->ps->requireRecompute)
	{
        ...
        for (int lineId = 0; lineId < numberOfCells; ++lineId)
        {
            ...
            double* p = (double*)malloc(sizeOfDouble3);
            double* tangent = (double*)malloc(sizeOfDouble3);
            double* tangentFlipped;
            if(ps->applyKernelInBothDirs)
                double* tangentFlipped = (double*)malloc(sizeOfDouble3);
            ...
            for (int pointId = 0; pointId < numberOfFiberPoints; ++pointId)
            {
                ...
                {
                    // load point
                    memcpy(p,&fiberData[fiberStartId + (pointId)*3],sizeOfDouble3);
                    //printf("loaded pos: {%f,%f,%f} \n", p[0],p[1],p[2]);

                    // load f1 tangent
                    memcpy(tangent,&fiberTangents[fiberStartId + (pointId)*3],sizeOfDouble3);
                    //printf("loaded tangent: {%f,%f,%f} \n", tangent[0],tangent[1],tangent[2]);
                    ...
                    for (int lineIdf2 = 0; lineIdf2 < numberOfCells; ++lineIdf2)
                    {
                        ...
                        double* pf2 = (double*)malloc(sizeOfDouble3);
                        double* tangentf2 = (double*)malloc(sizeOfDouble3);
                        ...
                        {
                            memcpy(pf2,&fiberData[fiberStartIdf2 + pointIdf2*3],sizeOfDouble3);
                            memcpy(tangentf2,&fiberTangents[fiberStartIdf2 + pointIdf2*3],sizeOfDouble3);

                            //printf("loaded pos f2: {%f,%f,%f} \n", pf2[0],pf2[1],pf2[2]);
                            //printf("loaded tangent f2: {%f,%f,%f} \n", tangentf2[0],tangentf2[1],tangentf2[2]);
